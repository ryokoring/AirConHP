...
export const getApp = cache(async (): Promise<AppMeta> => {
  const app = await client.getApp({
    appUid: process.env.NEXT_PUBLIC_NEWT_HOME_APP_UID + '',
  })
  return app
})
...
export const getConApp = cache(async (): Promise<AppMeta> => {
  const app = await client.getApp({
    appUid: process.env.NEXT_PUBLIC_NEWT_CON_APP_UID + '',
  })
  return app
})

export const getConArticles = cache(
  async (
    query?: GetContentsQuery,
  ): Promise<{ articles: Article[]; total: number }> => {
    const { items: articles, total } = await client.getContents<Article>({
      appUid: process.env.NEXT_PUBLIC_NEWT_CON_APP_UID + '',
      modelUid: process.env.NEXT_PUBLIC_NEWT_CON_ARTICLE_MODEL_UID + '',
      query: {
        depth: 2,
        ...query,
      },
...
export const getConArticle = cache(
...
export const getConPreviousArticle = cache(
...
export const getConNextArticle = cache(
  async (currentArticle: Article): Promise<{ slug: string } | null> => {
    const { createdAt } = currentArticle._sys
    const article = await client.getFirstContent<{ slug: string }>({
      appUid: process.env.NEXT_PUBLIC_NEWT_CON_APP_UID + '',
...
export const getConTags = cache(async (): Promise<TagWithCount[]> => {
  const { items: tags } = await client.getContents<Tag>({
    appUid: process.env.NEXT_PUBLIC_NEWT_CON_APP_UID + '',
    modelUid: process.env.NEXT_PUBLIC_NEWT_CON_TAG_MODEL_UID + '',
  })
...
export const getConTag = cache(async (slug: string): Promise<Tag | null> => {
  if (!slug) return null
...
export const getConAuthors = cache(async (): Promise<Author[]> => {
  const { items: authors } = await client.getContents<Author>({
    appUid: process.env.NEXT_PUBLIC_NEWT_CON_APP_UID + '',
    modelUid: process.env.NEXT_PUBLIC_NEWT_CON_AUTHOR_MODEL_UID + '',
  })
  ...
  const getConAuthorCount = (author: Author) => {
    return articles.filter((article) => {
  ...
  const validAuthors = authors.filter((author) => {
    // 1件も記事のない著者は除外
    return getConAuthorCount(author) > 0
  })

  return validAuthors
})

export const getConAuthor = cache(async (slug: string): Promise<Author | null> => {
  if (!slug) return null
...
export const getConArchives = cache(async (): Promise<Archive[]> => {
  ...
  const getConArchiveCount = (year: number) => {
    return articles.filter((article) => {
...
//corporate
export const getCorpApp = cache(async (): Promise<AppMeta> => {
  const app = await client.getApp({
