...
import { FacebookShareButton } from '@/components/ConAirCon/FacebookShareButton'
import { TwitterShareButton } from '@/components/ConAirCon/TwitterShareButton'
import { formatDate } from '@/lib/date'
import {
  getConArticles,
  getConArticle,
  getConPreviousArticle,
  getConNextArticle,
} from '@/lib/newt'
...
export async function generateStaticParams() {
  const { articles } = await getConArticles()
...
export async function generateMetadata({ params }: Props) {
  const { slug } = params
  const article = await getConArticle(slug)
...
export default async function Page({ params }: Props) {
  ...
  const prevArticle = await getConPreviousArticle(article)
  const nextArticle = await getConNextArticle(article)
